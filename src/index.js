...
// spends: 1 repos + 1 events + 1 search + N languages + 2 KV. Capping
// language lookups at 30 keeps the worst case at 35 with headroom.
...
async function aggregateStats(env, user) {
  // The three top-level calls are independent; running them in parallel
  // keeps cold-path latency near the slowest call instead of the sum.
  const ninetyDaysAgo = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10);

  const [repos, events, commitSearch] = await Promise.all([
    gh(env, `/users/${user}/repos?per_page=100&type=owner&sort=pushed`),
    gh(env, `/users/${user}/events/public?per_page=100`),
  ...
  return {
    ...
    recentCommits: extractRecentCommits(events, 10),
  };
}
...
/** Flatten PushEvents from the public event feed into a commit list. */
function extractRecentCommits(events, limit) {
  const commits = [];
